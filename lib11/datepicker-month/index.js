...
var MonthPicker = _regularjs2.default.extend({
	...
	show: function show() {
		...
		this.$update({
			...
			currentDay: currentDay ? currentDay : (0, _index3.formatTime)(thisDate, sdata.format || "yyyy-MM")
		});
	},
	hide: function hide() {
		this.$update({
	...
	__evSelect: function __evSelect(month, item) {
		...
		var date = (0, _index3.formatTime)(sdata.selectedYear + "/" + month + "/01", sdata.format || "yyyy-MM");
		self.$update({
			selectedMonth: month
		});
